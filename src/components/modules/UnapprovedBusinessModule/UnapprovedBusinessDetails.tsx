...
function BusinessDetailsInfoItem({ title, text }: BusinessDetailsInfoItemType) {
  return (
    <Typography sx={{ py: 1 }}>
      {title}: <span style={{ fontWeight: 600 }}>{text}</span>
    </Typography>
  );
}

export function UnapprovedBusinessDetails({
  unapproved,
  onRefetch,
}: UnapprovedBusinessDetailsProps) {
  ...
  return (
    <Paper sx={{ p: 2.5, bgcolor: paperBg }}>
      <BusinessDetailsInfoItem
        title="Tip Business"
        text={business.business_type.name}
      />
      <BusinessDetailsInfoItem
        title="Angajati"
        text={business.has_employees ? "DA" : "NU"}
      />
      <BusinessDetailsInfoItem
        title="Adresa"
